...
const linkedRenderObjsMethods = Object.assign(
  ...["lights", "cameraPosition", "postProcessingComposer"].map((p) => ({
  ...
  init: function (domNode, state) {
    ...
    state.renderObjs
      ...
      .tooltipContent((obj) => {
        ...
        return graphObj
          ? accessorFn(state[`${graphObj.__graphObjType}Label`])(
              graphObj.__data,
            ) || ""
          : "";
      })
      ...
      .onHover((obj) => {
        ...
        if (hoverObj !== state.hoverObj) {
          ...
          if (prevObjType && prevObjType !== objType) {
            // Hover out
            const fn =
              state[`on${prevObjType === "node" ? "Node" : "Link"}Hover`];
          ...
          renderer.domElement.classList[
            (hoverObj &&
              state[`on${objType === "node" ? "Node" : "Link"}Click`]) ||
